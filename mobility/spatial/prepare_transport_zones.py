...
def _get_sidecar_paths(output_fp: pathlib.Path) -> tuple[pathlib.Path, pathlib.Path]:
    stem = output_fp.name.replace("-transport_zones.gpkg", "")
    return (
        output_fp.parent / f"{stem}-transport_zones_buildings.parquet",
        output_fp.parent / f"{stem}-transport_zones_buildings_geoms.gpkg",
    )
