...
def radiation_model(sources, sinks, costs, alpha=0, beta=1):
    ...
    matrix_origin_destinations = pd.merge(matrix_origin_destinations, temp, left_on='from', right_index=True)
...
def iter_radiation_model(sources, sinks, costs, alpha=0, beta=1, max_iter=20, plot=False):
    """
    ...
    while iteration<max_iter and flows['flow_volume'].sum()>0.01*total_source_volume :
        ...
        flows, source_volume, sink_volume = radiation_model(source_volume, sink_volume, 
                                                            costs,alpha=alpha, beta=beta)
...
def plot_volume(volume_location, coordinates, n_locations=10, title=""):
    ...
    for idx in idx_show:
        plt.text(volume_location.loc[idx, 'x'], volume_location.loc[idx, 'y'],
...
def plot_flow(flows, coordinates, sources=None, n_flows=100, n_locations=5, size=1, title=""):
    ...
    # Normalization
    flows['flow_volume'] = size*100*(flows['flow_volume']-flows['flow_volume'].min()) / flows['flow_volume'].max()
    ...
    if sources is None :
        ...
        # Normalization
        internal_flows.loc[:, 'flow_volume'] = size**2*100*(internal_flows.loc[: ,'flow_volume']-internal_flows['flow_volume'].min()) / internal_flows['flow_volume'].max()
        plt.scatter(internal_flows['from_x'], internal_flows['from_y'], s=internal_flows['flow_volume'], zorder=1)
        ...
        for idx in idx_show:
            plt.text(coordinates.loc[idx, 'x'], coordinates.loc[idx, 'y'],
                     coordinates.loc[idx, 'NOM_COM'][0:14], fontsize=5*size)
    else :
        # Normalization
        sources['source_volume'] = size**2*100*(sources['source_volume']-sources['source_volume'].min()) / sources['source_volume'].max()
        ...
        for idx in idx_show:
            plt.text(sources.loc[idx, 'x'], sources.loc[idx, 'y'],
                     sources.loc[idx, 'NOM_COM'][0:14], fontsize=5*size)
