...
def get_data_for_model_school(
    ...
    """
    Gets data for the given departments, for a given categories of students.

    Uses the get_insee_data function for student population ans schools
    Uses local data for communes' superficies, intra-communal distance
    and work-home mobility
    (by default, will use the national CSVs in the directory)
    ...
    lst_departments : list
        List of departements for which to get the data.
    Age : int
        Represent the choseen years of study
        1 refeers to 'école', 2 to 'collège' and 3 to 'lycée'
    communes_coordinates_csv : string, optional
        Which CSV to use for coordinates data.
        The default is COMMUNES_COORDINATES_CSV.
    communes_surfaces_csv : string, optional
        Which CSV to use for surfaces data.
        The default is COMMUNES_SURFACES_CSV.
    school_home_fluxes_csv : string, optional
        Which CSV to use for school-home data.
        The default is school_home_fluxes_csv defined at the top of the file.
    alpha : float, optional
        The default is 0.
    beta : float, optional
        The default is 1.
    ...
    # Import the data (student population ans schools)
    insee_data = get_insee_data(test=test)
    ...
    # Import the INSEE data on the school-home mobility on Millau
