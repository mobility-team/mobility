"""Describe a service and convert it to GTFS tables and archives."""
...
class GTFSLineSpec:
    ...
    def validate(self) -> None:
        ...
        if not all(math.isfinite(value) for value in (
            self.start_time, self.end_time, self.period, *self.segment_travel_times,
        )):
            raise ValueError("Line times and departure interval must be finite numbers of seconds.")
        if self.start_time < 0:
            raise ValueError("Line start time must be zero or more seconds after midnight.")
...
def _create_gtfs_tables(feed: GTFSFeedSpec) -> dict[str, pd.DataFrame]:
    ...
    stops = pd.DataFrame(
        [
            ...
            for stop_id, stop in sorted(feed.stops.items())
        ]
    )
...
def _write_gtfs_zip(tables: Mapping[str, pd.DataFrame], path: Path) -> None:
    ...
    with zipfile.ZipFile(path, "w") as archive:
        for name, table in sorted(tables.items()):
            # Fixed ZIP dates and CSV newlines keep generated bytes reproducible.
            entry = zipfile.ZipInfo(name)
            entry.compress_type = zipfile.ZIP_DEFLATED
            with archive.open(entry, "w") as member, io.TextIOWrapper(member, encoding="utf-8", newline="") as stream:
