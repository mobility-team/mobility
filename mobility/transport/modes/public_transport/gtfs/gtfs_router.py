...
from mobility.transport.modes.public_transport.gtfs.gtfs_stops import GTFSStops
...
class GTFSRouter(FileAsset):
    ...
    def prepare_gtfs_router(self, transport_zones: TransportZones, gtfs_files: list[str]) -> None:
        ...
        script = RScriptRunner(
            resources.files("mobility.transport.modes.public_transport.gtfs").joinpath("prepare_gtfs_router.R")
        )
        script.run(
            args=[
