...
from mobility.choice_models.evaluation.public_transport_network_evaluation import (
...
class PopulationTripsRunResults:
    """Run-scoped analysis helper for one `PopulationTripsRun` output set."""
    ...
    def global_metrics(self, normalize: bool = True):
        ...
        def aggregate(df):
            return (
                df.filter(pl.col("motive_seq_id") != 0)
        ...
        trip_count = aggregate(self.states_steps)
    ...
    def immobility(self, plot: bool = True):
        ...
        immobility = (
            self.states_steps.filter(pl.col("motive_seq_id") == 0)
            .rename({"home_zone_id": "transport_zone_id"})
            .join(transport_zones_df.select(["transport_zone_id", "local_admin_unit_id"]), on=["transport_zone_id"])
    ...
    def sink_occupation(self, plot_motive: str = None, mask_outliers: bool = False):
        """Compute sink occupation per zone and motive for this run."""
        transport_zones_df = (
            pl.DataFrame(self.transport_zones.get().drop("geometry", axis=1))
            .filter(pl.col("is_inner_zone"))
            .lazy()
        ...
        sink_occupation = (
            ...
            .join(self.sinks.select(["to", "motive", "sink_capacity"]), on=["to", "motive"])
            .with_columns(sink_occupation=pl.col("duration") / pl.col("sink_capacity"))
    ...
    def metric_per_person(
        ...
        if compare_with is not None:
            try:
                ...
                states_steps_comp = pl.scan_parquet(compare_with.cache_path[f"{prefix}_flows"])
                costs_comp = pl.scan_parquet(compare_with.cache_path[f"{prefix}_costs"])
            except Exception:
                raise Exception("The PopulationTrips to compare with did not work. Try to run it alone?")

            metric_comp = metric + "_comp"
            metric_per_person_comp = metric + "_per_person_comp"
            metric_per_groups_and_transport_zones_comp = (
                states_steps_comp.filter(pl.col("motive_seq_id") != 0)
                .rename({"home_zone_id": "transport_zone_id"})
    ...
    def plot_modal_share(self, zone="origin", mode="car", labels=None, labels_size=[10, 6, 4], labels_color="black"):
        ...
        population_df = self.states_steps.collect().to_pandas()
