...
def trips_share(trips, var="trip_id"):
    ...
    for variable in ["city_category", "csp", "motive_group", "mode_group"]:
        ...
        for i in trips["survey"].unique().tolist():
            average_dist[i] = trips.loc[trips["survey"] == i, "distance"].sum() / trips.loc[
                trips["survey"] == i, "n_trips"].sum()
        trips = pd.merge(trips,
                         trips.groupby(["survey"], as_index=False)["n_trips"].sum().rename(
                             columns={"n_trips": "n_trips_tot"}),
                         on="survey",
                         how="left")
    ...
    for i in trips_source["survey"].unique().tolist():
        print("La distance moyenne dans la bdd " + i + " est de " + str(int(average_dist[i])) + " km." + add_str)
        ...
        for day_type in ["all", "weekday", "weekend"]:
            ...
            for variable in ["motive_group", "mode_group"]:
                ...
                fig.update_layout(
                    ...
                    template="simple_white",
                    title={
                        'text': "Distance moyenne journalière en fonction des " + variable_titles[
                            variable] + add_str + filter_str,
                        'y': 0.95,
                        'x': 0.5,
                        'xanchor': 'center',
                        'yanchor': 'top'},
...
def carbon_and_distance(trips):
    ...
    trips = trips.groupby(["survey", "motive_group"], as_index=False).agg(
        {"distance": ['sum'], "emissions": ['sum'], "pondki": ['sum']})
