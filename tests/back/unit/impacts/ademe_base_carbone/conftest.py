...
def patch_asset_init(
    ...
    """
    Stub mobility.runtime.assets.asset.Asset.__init__ so it does NOT call .get().
    Creates a minimal Asset class if mobility.runtime.assets.asset is not importable to avoid ImportErrors.
    Sets:
      - self.inputs
      - self.inputs_hash
      - self.cache_path
    ...
    if "mobility.runtime.assets.asset" not in sys.modules:
        asset_module = types.ModuleType("mobility.runtime.assets.asset")
        sys.modules["mobility.runtime.assets.asset"] = asset_module

    import importlib

    asset_mod = importlib.import_module("mobility.runtime.assets.asset")
...
# ---------- Helpers to seed IndividualYearTrips-like instances for direct method tests ----------

@pytest.fixture
def seed_trips_helpers():
    """
    Provide helpers to seed attributes on an IndividualYearTrips instance for tests that call
