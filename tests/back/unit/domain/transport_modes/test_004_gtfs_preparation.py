...
from mobility.transport.modes.public_transport.gtfs.prepare_gtfs import prepare_gtfs
...
def test_dates_only_and_long_route_name(tmp_path, feed_files, zones, route_types):
    ...
    tables, metadata = prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)
...
def test_no_tuesday_does_not_restore_trips(tmp_path, feed_files, zones, route_types, cancelled):
    ...
    with pytest.raises(ValueError, match="No active Tuesday"):
        prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)


def test_frequency_and_restrictions_survive(tmp_path, feed_files, zones, route_types):
    feed_files["frequencies"] = "trip_id,start_time,end_time,headway_secs,exact_times\nt,08:00:00,10:00:00,600,1\n"
    feed_files["stop_times"] = "trip_id,arrival_time,departure_time,stop_id,stop_sequence,pickup_type,drop_off_type\nt,08:00:00,08:00:00,a,1,1,0\nt,08:10:00,08:10:00,b,2,0,1\n"
    tables, _ = prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)
...
def test_intermediate_time_is_interpolated(tmp_path, feed_files, zones, route_types):
    ...
    feed_files["stop_times"] = "trip_id,arrival_time,departure_time,stop_id,stop_sequence,timepoint\nt,08:00:00,08:00:00,a,1,1\nt,,,c,2,0\nt,08:10:00,08:10:00,b,3,1\n"
    tables, _ = prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)
...
def test_duplicate_feed_and_original_dates(tmp_path, feed_files, zones, route_types):
    ...
    feed_files["calendar"] = feed_files["calendar"].replace("20260908,20260915", "20261006,20261006")
    future = write_feed(tmp_path / "future.zip", feed_files)
    tables, metadata = prepare_gtfs([first, duplicate, future], zones, route_types)
    assert metadata["selected_date"] == "2026-09-08"
...
def test_stop_visits_outweigh_calendar_ids(tmp_path, feed_files, zones, route_types):
    ...
    feed_files["stops"] += "c,C,48.11,-1.67995\n"
    feed_files["stop_times"] += "t2,08:00:00,08:00:00,a,1\nt2,08:05:00,08:05:00,c,2\nt2,08:10:00,08:10:00,b,3\nq1,08:00:00,08:00:00,a,1\nq1,08:10:00,08:10:00,b,2\nq2,08:00:00,08:00:00,a,1\nq2,08:10:00,08:10:00,b,2\n"
    feed_files["calendar"] = feed_files["calendar"].replace("20260908,20260915", "20260908,20260908")
    feed_files["calendar"] += "q1,0,1,0,0,0,0,0,20260915,20260915\nq2,0,1,0,0,0,0,0,20260915,20260915\n"
    tables, metadata = prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)
    assert metadata["selected_date"] == "2026-09-08"
...
def test_forbidden_and_route_specific_transfers_preserved(tmp_path, feed_files, zones, route_types):
    feed_files["transfers"] = "from_stop_id,to_stop_id,transfer_type,min_transfer_time,from_route_id\na,b,3,,\na,b,2,600,r\n"
    tables, _ = prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)
...
def test_one_stop_feed_is_discarded(tmp_path, feed_files, zones, route_types):
    ...
    feed_files["stop_times"] = "trip_id,arrival_time,departure_time,stop_id,stop_sequence\nt,08:00:00,08:00:00,a,1\n"
    bad = write_feed(tmp_path / "one-stop.zip", feed_files)
    tables, metadata = prepare_gtfs([good, bad], zones, route_types)
    assert metadata["sources"][1]["status"] == "no usable trips in area"
    assert len(tables["trips"]) == 1


def test_asset_writes_manifest_and_invalidates_changed_manual_feed(tmp_path, feed_files, zones, monkeypatch):
...
def test_r_graph_reads_parquet_and_respects_direction(tmp_path, feed_files, zones, route_types, case):
    ...
    script_path = resources.files("mobility.transport.modes.public_transport").joinpath("prepare_public_transport_graph.R")
...
'''
        script += "stopifnot(nrow(connection) == 0)\n" if case == "forbidden" else "stopifnot(nrow(connection) == 1, connection$transfer_time == 600)\n"
    test_script = tmp_path / "check_graph.R"
    test_script.write_text(script)
    output = tmp_path / "graph" / "simplified" / "fixture-graph"
    output.parent.mkdir(parents=True)
    result = subprocess.run(
        [rscript, str(test_script), str(resources.files("mobility")), "unused", str(manifest),
         json.dumps({"start_time_min": 7, "start_time_max": 10, "wait_time_coeff": 2,
                     "transfer_time_coeff": 2, "no_show_perceived_prob": 0.2, "target_time": 9}),
...
def test_calendar_exception_adds_service_on_a_nonweekly_tuesday(tmp_path, feed_files, zones, route_types):
    feed_files["calendar"] = feed_files["calendar"].replace("s,0,1,0,0,0,0,0", "s,0,0,0,0,0,0,1")
    feed_files["calendar_dates"] = "service_id,date,exception_type\ns,20260915,1\n"
    _, metadata = prepare_gtfs([write_feed(tmp_path / "feed.zip", feed_files)], zones, route_types)
...
def test_trip_specific_transfer_fails_explicitly(tmp_path, feed_files, zones, route_types):
    feed_files["transfers"] = "from_stop_id,to_stop_id,transfer_type,min_transfer_time,from_trip_id\na,b,2,60,t\n"
