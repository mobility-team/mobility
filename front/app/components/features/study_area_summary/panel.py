...
def StudyAreaSummary(zones_gdf, visible=True, id_prefix="map", header_offset_px=80, width_px=340):
    ...
    if zones_gdf is None or getattr(zones_gdf, "empty", True):
        content = dmc.Text("Données globales indisponibles.", size="sm",
        ...
        content = dmc.Stack(
            [
                ...
                ModalSplitList(
                    share_car=share_car, share_bike=share_bike, share_walk=share_walk, share_carpool=share_pool,
                    share_pt=share_pt, share_pt_walk=share_pt_walk, share_pt_car=share_pt_car,
    ...
    return html.Div(
        id=comp_id,
        children=dmc.Paper(content, withBorder=True, shadow="md", radius="md", p="md",
                           style={"width": "100%", "height": "100%", "overflowY": "auto",
                                  "background": "#ffffffee", "boxSizing": "border-box"}),
        style={"display": "block" if visible else "none", "position": "absolute",
               "top": f"{header_offset_px}px", "right": "0px", "bottom": "0px",
               "width": f"{width_px}px", "zIndex": 1200, "pointerEvents": "auto", "overflow": "hidden"},
    )
